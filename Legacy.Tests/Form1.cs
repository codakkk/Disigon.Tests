    ...
    public partial class Form1 : Form
    {
        ...
        private void signTextButton_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                SignedDocument sd = dsign.Sign(Encoding.Default.GetBytes(textToSign.Text), new QualifiedCertificateSelector());
        ...
        private void signFileButton_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (textBoxP11.Text.ToLower().Equals("auto"))
                {
                    try
                    {
                        p11dll = "bit4xpki.dll";// DigitalSigner.GetMatchingCryptokiPath();
                    }
                    catch (DigitalSignatureException)
                    {
                        MessageBox.Show("No Matching pkcs11 dll found. Try to set the PKCS#11 dll instead of use 'auto'");
                ...
                DigitalSigner dsign = new P11DigitalSigner();
        ...
        private void verifyButton_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                FileStream fs = new FileStream(textBoxFileToVerify.Text, FileMode.Open);
                byte[] data = new byte[fs.Length];
                fs.Read(data, 0, data.Length);
                fs.Close();

                SignedDocument signedDoc = SignedDocument.FromByteArray(data);

                int signerCount = signedDoc.SignersCount;
                ...
                for (int i = 0; i < signerCount; i++)
                {
                    ...
                    {                      
                        try
                            {
                            DateTime revocatioDate;
                                //RevocationStatus status0 = OCSPVerifier.CheckRevocationStatus(cert);
                                RevocationStatus status = CRLVerifier.CheckRevocationStatus(cert);
                            ...
                            else if (status == RevocationStatus.Revoked)
                            {
                                ...
                                revocatioDate = CRLVerifier.RevocationDate;
        ...
        private void signPDF_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                pdfSigner.SignatureImg = new Bitmap(@"..\..\pen_signature_red.png");
